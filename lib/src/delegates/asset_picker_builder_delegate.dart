...
// ignore_for_file: use_build_context_synchronously, unawaited_futures, join_return_with_assignment
...
import 'package:media_picker/src/internals/singleton.dart';
...
abstract class AssetPickerBuilderDelegate<Asset, Path> {
  ...
  TextDirection effectiveGridDirection(BuildContext context) {
    final od = Directionality.of(context);
...
class DefaultAssetPickerBuilderDelegate
  ...
  Future<void> selectAsset(
    ...
    final selectPredicateResult = await selectPredicate?.call(
      context,
      asset,
      selected,
    );
    ...
    if (isSingleAssetMode && !isPreviewEnabled) {
      await Navigator.of(context).maybePop(provider.selectedAssets);
    }
  }
  ...
  Future<void> viewAsset(
    ...
    final result = await AssetPickerViewer.pushToViewer(
      context,
  ...
  Widget assetsGridBuilder(BuildContext context) {
    ...
    final gridRevert = effectiveShouldRevertGrid(context);
    return Selector<DefaultAssetPickerProvider, PathWrapper<AssetPathEntity>?>(
      ...
      builder: (
        ...
        var totalCount = wrapper?.assetCount ?? 0;
        ...
        final row = (totalCount + placeholderCount) ~/ gridCount;
        ...
        final dividedSpacing = itemSpacing / gridCount;
        final topPadding = context.topPadding + appBarPreferredSize!.height;
        ...
        return LayoutBuilder(
          builder: (BuildContext context, BoxConstraints constraints) {
            final itemSize = constraints.maxWidth / gridCount;
            // Check whether all rows can be placed at the same time.
            final onlyOneScreen = row * itemSize <=
  ...
  Widget assetGridItemBuilder(
    ...
    final length = currentAssets.length;
    final currentWrapper = p.currentPath;
    final currentPathEntity = currentWrapper?.path;
    ...
    final asset = currentAssets.elementAt(currentIndex);
    final builder = switch (asset.type) {
      AssetType.image ||
      AssetType.video =>
    ...
    return ValueListenableBuilder<bool>(
      ...
      builder: (_, bool isSwitchingPath, Widget? child) {
        return Consumer<DefaultAssetPickerProvider>(
          builder: (_, DefaultAssetPickerProvider p, __) {
            final isBanned = (!p.selectedAssets.contains(asset) &&
                    p.selectedMaximumAssets) ||
                (isWeChatMoment &&
                    asset.type == AssetType.video &&
                    p.selectedAssets.isNotEmpty);
            final isSelected = p.selectedDescriptions.contains(
              asset.toString(),
            );
            final selectedIndex = p.selectedAssets.indexOf(asset) + 1;
            var hint = '';
  ...
  int findChildIndexBuilder({
    ...
    var index = assets.indexWhere((AssetEntity e) => e.id == id);
    if (specialItemPosition == SpecialItemPosition.prepend) {
      index += 1;
    }
    index += placeholderCount;
  ...
  Widget pathEntityWidget({
    ...
    final pathName = pathNameBuilder?.call(pathEntity) ?? pathEntity.name;
    final name = isPermissionLimited && pathEntity.isAll
        ? textDelegate.accessiblePathName
        : pathName;
    final semanticsName = isPermissionLimited && pathEntity.isAll
        ? semanticsTextDelegate.accessiblePathName
        : pathName;
    final semanticsCount = wrapper.assetCount?.toString();
    ...
    return Selector<DefaultAssetPickerProvider, PathWrapper<AssetPathEntity>?>(
      ...
      builder: (_, PathWrapper<AssetPathEntity>? currentWrapper, __) {
        final isSelected = currentWrapper?.path == pathEntity;
        return Semantics(
  ...
  Widget selectedBackdrop(BuildContext context, int index, AssetEntity asset) {
    ...
    return Positioned.fill(
      child: GestureDetector(
        ...
        child: Consumer<DefaultAssetPickerProvider>(
          builder: (_, DefaultAssetPickerProvider p, __) {
            final index = p.selectedAssets.indexOf(asset);
