...
// ignore_for_file: invalid_return_type_for_catch_error, cascade_invocations, prefer_asserts_with_message
...
import 'package:media_picker/src/delegates/sort_path_delegate.dart';
import 'package:media_picker/src/internals/singleton.dart';
import 'package:media_picker/src/models/path_wrapper.dart';
...
abstract class AssetPickerProvider<Asset, Path> extends ChangeNotifier {
  ...
  void setPathThumbnail(Path path, Uint8List? data) {
    ...
    if (index != -1) {
      final newWrapper = _paths[index].copyWith(
        thumbnailData: data,
      );
  ...
  void selectAsset(Asset item) {
    ...
    final set = selectedAssets.toList();
    set.add(item);
    selectedAssets = set;
  }
...
class DefaultAssetPickerProvider
  ...
  Future<void> getAssetCountFromPath(PathWrapper<AssetPathEntity> path) async {
    final assetCount = await path.path.assetCountAsync;
    final index = _paths.indexWhere(
